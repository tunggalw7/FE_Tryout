...
@Component({
  selector: 'grafik-siswa-ips',
  styleUrls: ['./grafik-siswa-ips.component.scss'],
  templateUrl: './grafik-siswa-ips.component.html',
})

export class GrafikTOIpsSiswaComponent implements OnDestroy, OnInit {
  baseService = "core/to-ips";
  ubService = "core/uang-biaya";
  ...
  ngOnInit() {    
    ...
    if (this.route.snapshot.params.nis != null) {
      ...
      this.nis = this.route.snapshot.params.nis;
      var d = new Date();
      ...
      this.getListBayarSiswa(this.nis);
      this.getEventList();
    }
  }
  ...
  getDetail(id){    
    this.masterService.getByID(this.siswaService, id).subscribe((result) => {
      ...
      this.total_biaya = this.detailSiswa.total_biaya;
  ...
  getListBayarSiswa(nis){    
    this.masterService.getByID(this.ubService, nis).subscribe(
      res => {
        debugger
        ...
        if (this.dataTable){
          ...
          this.sisa_biaya_kurang = this.dataTable[this.dataTable.length-1].biaya_kurang;
        }
      },
      err => {
  ...
  async getDetailSiswa(nis){    
    this.masterService.getByID(this.baseService, nis).subscribe(
      res => {
        this.dataForm = res['payload'] ? res['payload'][0] : '';
        this.dataAPI = res['payload'];
        if (this.dataAPI){
          this.onChangeMenu('TOTAL');
          this.temp = [...this.dataAPI];
          this.rows = this.dataAPI;
        }
        this.getSoalList(this.kelas);
      },
      err => {
        this.showToast('top-right', 'danger', 'Something went wrong, please try again later.');
      },
  ...
  async getQuizPerMatpel() {   
    ...
    const end_date = this.tanggalQuizTable.end ? moment(this.tanggalQuizTable.end).format("YYYY-MM-DD") : start_date;
